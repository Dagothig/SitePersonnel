{ // SPRITES
	...
	sprLargeNet.src = "Assets/LargeNet.png";
...
{ // CLASSES
					...
					if (this.AnimY == 4)
					{
						...
						for (var index = Fishes.length - 1; index >= 0; index--)
						{
							if (fishes + Fishes[index].Size / 24 <= this.Capacity &&
								this.X + this.Width - 4 > Fishes[index].X &&
		...
		function Cannonball(pImage, pX, pY, pSpeedX, pSpeedY, pSize)
		{
			...
			Cannonball.prototype.ColCheck = function()
			{
				for (var index = 0; index < Ships.length; index++)
					...
					{
						this.Dispose();
						Ships[index].Health -= (Math.abs(this.MovX - Ships[index].MovX) * pSize) / 25;
	...
	function Ship(pHull, pX, pY)
	{
		...
		Ship.prototype.TryCatch = function()
		{
			...
			if (this.Net.AnimY == -1)
			{
				this.Net.TryCatch(this, 48, this.Hull.Height * 0.5 - this.Net.Height * 0.36);
			}
		}
		...
		Ship.prototype.Update = function()
		{
			...
			this.MovY += 1.5;
			if (this.Y + this.Hull.Height * 0.5 > seaLevel - highestWave)
				this.MovY -= 2;
			} // end WAVES
			{ // MOVEMENT
		...
		Ship.prototype.AI = function()
		{
			...
			else
			{
				...
				if (this.AINextPoint == undefined || Math.abs(this.X - this.AINextPoint) < this.Hull.Width * 0.25)
					...
					{
						this.AINextPoint = (Math.random() - 0.5) * 25600 + Player.X;
					}
...
{ // GAME VARIABLES
	...
	{ // Small wooden hull
		...
		new Hull(sprSmallWoodHull, 240, 120, swhMasts, swhOarPositions, swhCannonPositions, 80, 15, 1, "Small Wood Hull", 5);
	}
	{ // Large wooden hull
		swhMasts = new Array();
		swhMasts.push(new Mast(sprSmallWoodMast, 64, -100, 96, 120));
		swhOarPositions = new Array();
		swhOarPositions.push(new Point(48, 108));
		swhOarPositions.push(new Point(88, 108));
		swhOarPositions.push(new Point(168, 108));
		swhOarPositions.push(new Point(208, 108));
		...
		new Hull(sprLargeWoodHull, 280, 180, swhMasts, swhOarPositions, swhCannonPositions, 200, 40, 0.4, "Large Wood Hull", 12);
	}	
	{ // Long hull
		swhMasts = new Array();
		...
		new Hull(sprLongHull, 250, 100, swhMasts, swhOarPositions, swhCannonPositions, 160, 25, 0.5, "Long Hull", 8.5);
	}
	
	new NetType(sprNet, 120, 120, 1, "Tiny Net", 1.5);
	new NetType(sprNet, 120, 120, 2, "Small Net", 2.5);
	new NetType(sprLargeNet, 160, 160, 5, "Large Net", 4.5);
	new NetType(sprLargeNet, 160, 160, 10, "Huge Net", 6.5);
	...
	new CannonType(sprAverageCannon, 48, 32, sprAverageShot, 12, 30, 4, 180, "Average Cannon", 6);
	new CannonType(sprBombard, 54, 41, sprBombardShot, 24, 75, 12, 240, "Bombard", 14);
	...
	var Draw = function()
	{
		...
		if (InShop != undefined)
		{
			...
			else if (ShopMenu == "buy")
			{
				...
				Context.font="12pt Arial";
				Context.fillText(ShopSubMenu + ':', 244, 244);
		...
		if (!Pause)
		{
			...
			if (Player instanceof Ship)
				if (Player.Orientation == 1)
					CamX -= (CamX - (Player.X - Player.MovX)) / 12;
				else
					CamX -= (CamX - (Player.X - Player.MovX + Player.Hull.Width - 640)) / 12;
			Player.Moving = 0;
			// Player input
			if (ctrLeft)
			{
